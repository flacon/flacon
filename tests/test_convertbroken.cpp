 ...
 *
 ************************************************/
void TestFlacon::testConvertBroken()
{
 ...
 *
 ************************************************/
void TestFlacon::testConvertBroken_data()
{
    if (QProcessEnvironment::systemEnvironment().contains("FLACON_SKIP_CONVERT_TEST")) {
        QTest::qSkip("Skipping testConvertBroken", __FILE__, __LINE__);
    }

    QString curDir = QDir::currentPath();

    QTest::addColumn<QString>("dataDir", nullptr);
    QString dataDir = mDataDir + "testConvertBroken";

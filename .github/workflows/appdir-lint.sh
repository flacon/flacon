...
if [ ! -e "${HERE}/appdir-lint-excludelist" ] ; then
  fatal "appdir-lint-excludelist missing, please install it"
fi
...
BLACKLISTED_FILES=$(cat "${HERE}/appdir-lint-excludelist" | sed '/^\s*$/d ; /^#.*$/d ; s/\s*#.*$//')
